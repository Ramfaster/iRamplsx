...
// 상단 알람 ticker 설정
function initAlarmListTicker ()
{
    var $alarmTicker = $ ( '#alarm_ticker' );
    var tpl = getTemplate ( templates.alarmListTickerLi );

    showAlarmListTicker ( $alarmTicker, tpl, true );
    choiceAlarmAck ( $alarmTicker );
    setInterval ( function ()
    {
        showAlarmListTicker ( $alarmTicker, tpl, false );
    }, 1000 * 3 );
}

// 알람 ticker 보이기
function showAlarmListTicker ( $alarmTicker, tpl, firstFlag )
{
    if ( $alarmTicker.size () !== 0 )
    {
        ...
        var vTickerWork = $alarmTicker.vTicker ( 'work' );

        if ( pvAlarmBannerList === null || pvAlarmBannerList.length === 0 )
        {
            if ( vTickerWork )
            {
                $alarmTicker.vTicker ( 'remove' );
            }

            $alarmTicker.find ( '.btn_chk_wrap' ).hide ();
            $alarmTicker.next ( '.notification' ).find ( '.mark' ).text ( 0 );
        } else
        {
            var isEqual = true;

            if ( $alarmTicker.data ( 'pv-alarm-banner-list' ) === null )
            {
                isEqual = false;
            } else
            {
                var prevPvAlarmBannerList = $alarmTicker.data ( 'pv-alarm-banner-list' );

                if ( (typeof prevPvAlarmBannerList === 'undefined' || prevPvAlarmBannerList === null)
                        || (typeof prevPvAlarmBannerList !== 'undefined' && prevPvAlarmBannerList !== null && prevPvAlarmBannerList.length !== pvAlarmBannerList.length) )
                {
                    isEqual = false;
                } else
                {
                    for ( var i = 0, length = prevPvAlarmBannerList.length; i < length; i++ )
                    {
                        if ( !_.isEqual ( prevPvAlarmBannerList[i], pvAlarmBannerList[i] ) )
                        {
                            isEqual = false;
                            break;
                        }
            ...
            if ( !isEqual )
            {
                if ( tpl !== null )
                {
                    ...
                    var html = template ( {
                        ...
                        lang : lang,
                        locale : locale,
                        pvAlarmBannerList : pvAlarmBannerList
                    } );
                    ...
                    $alarmTicker.data ( 'pv-alarm-banner-list', pvAlarmBannerList ).find ( '.btn_chk_wrap' ).show ();
                    $alarmTicker.find ( alarmListId ).html ( html );
                    $alarmTicker.vTicker ( 'init', {
                        pause : 3000,
                        speed : 1000,
                        height : 44,
                        removeElement : alarmListId
                    } );

                    $alarmTicker.next ( '.notification' ).find ( '.mark' ).html ( alarmCountView ).data (
                            'alarm-count', alarmCount );

                    $alarmTicker.find ( '.link_alarm_banner' ).magnificPopup (
                            {
                                type : 'ajax',
                                alignTop : false,
                                overflowY : 'scroll',
                                ...
                                callbacks : {
                                    open : function ()
                                    {
                                        ...
                                        var alarmCount = parseInt ( $mark.data ( 'alarm-count' ), 10 );
                                        ...
                                        {
                                            $ (
                                                    '#alarm_list li[data-occrrnc-id="'
                                                            + $.magnificPopup.instance.st.el.closest ( 'li' ).data (
                                                                    'occrrnc-id' ) + '"]' ).remove ();
                                        }
                                    ...
                                    {
                                        setParameter ( $alarmTicker.find ( '.link_alarm_banner' ) );
                                    }
                                }
                            } );
                }
...
// 알람 확인 처리
function choiceAlarmAck ( $alarmTicker )
{
    var $btnSingleAck = $ ( '#btn_single_ack' );
    var $btnAllAck = $ ( '#btn_all_ack' );

    $btnSingleAck.on ( 'mouseenter', function ()
    {
        $alarmTicker.vTicker ( 'pause', true );
    } ).on ( 'mouseleave', function ()
    {
        $alarmTicker.vTicker ( 'pause', false );
    } ).on ( 'click', function ()
    {
        var $alarmLi = $alarmTicker.find ( '#alarm_list li' ).eq ( 0 );

        if ( $alarmLi.size () > 0 )
        {
            var occrrncId = $alarmLi.find ( '.link_alarm_banner' ).data ( 'occrrnc-id' );
            var tagId = $alarmLi.find ( '.link_alarm_banner' ).data ( 'tag-id' );
            var alarmGradCd = $alarmLi.find ( '.link_alarm_banner' ).data ( 'alarm-grad-cd' );

            var params = {
                occrrncIds : occrrncId,
                tagIds : tagId,
                alarmGradCds : alarmGradCd
            };

            $.ajax ( {
                url : contextPath + '/hom/common/setOccrrncAlarmAck.ajax',
                type : 'POST',
                data : params,
                dataType : 'json',
                success : function ( json )
                {
                    if ( json.status === staticVariable.jsonStatusSuccess )
                    {
                        ...
                        {
                            $ ( '#alarm_list li[data-occrrnc-id="' + $alarmLi.data ( 'occrrnc-id' ) + '"]' ).remove ();
                        }
    ...
    $btnAllAck.on ( 'mouseenter', function ()
    {
        $alarmTicker.vTicker ( 'pause', true );
    } ).on ( 'mouseleave', function ()
    {
    ...
    {
        var $alarmLi = $alarmTicker.find ( '#alarm_list li' );
        ...
        if ( $alarmLi.size () > 0 )
        {
            ...
            $alarmLi.each ( function ()
            {
                occrrncIds.push ( $ ( this ).find ( '.link_alarm_banner' ).data ( 'occrrnc-id' ) );
                tagIds.push ( $ ( this ).find ( '.link_alarm_banner' ).data ( 'tag-id' ) );
                alarmGradCds.push ( $ ( this ).find ( '.link_alarm_banner' ).data ( 'alarm-grad-cd' ) );
            } );

            var params = {
                occrrncIds : occrrncIds,
                tagIds : tagIds,
                alarmGradCds : alarmGradCds
            };
            ...
            $.ajax ( {
                ...
                success : function ( json )
                {
                    ...
                    {
                        $alarmLi.remove ();
                        $ ( '#btn_alarm_count' ).find ( '.mark' ).html ( '0' );
                        $alarmTicker.find ( '.btn_chk_wrap' ).hide ();
                    } else if ( json.status === staticVariable.jsonStatusError )
                    {
                        $.customizeDialog ( {
