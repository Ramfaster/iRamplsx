function btncopyitemcheck ()
{
    if ( typeof $ ( "#btn_copy_popup_exec" ) !== 'undefined' )
    {
        $ ( "#btn_copy_popup_exec" ).on ( 'click', function ()
        {
            ...
            {
                $.ajax ( {
                    url : contextPath + parmUriPath + '/copyItemPopup.ajax',
                    type : 'POST',
                    data : params,
                    dataType : 'json',
                        ...
                        if ( json.status === staticVariable.jsonStatusSuccess )
                        {
                            ...
                            } else
                            {
                                ...
                                if ( pvTreeNode != null )
                                {
                                    ...
                                    showTreeLocation ( zTree, pvTreeNode );

                                    if ( pvTreeNode.level == 1 )
                                    {
                                        mntrcItmMgt.itemId = '';
                                        mntrcItmMgt.pvId = pvTreeNode.id;
                                        mntrcItmMgt.level = pvTreeNode.level;
...
function fnPopupCopyTextValue ()
{
    $ ( "#div_pop_subtxt01" ).text (
            $ ( ':selected', $ ( '#popupCopyPvId1' ) ).text () + ' > '
                    + $ ( ':selected', $ ( '#popupCopyPvId2' ) ).text () + ' ' + i18nMessage.msg_mntrcItmCopy );
}
