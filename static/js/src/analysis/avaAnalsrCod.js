...
{
    var noDataId = 'avaAnlsr_cod_grid_nodata';

    codJqGrid ( noDataId );

    var $avaAnlsrCodGridList = $ ( '#avaAnlsr_cod_gridList' );
    ...
    if ( avaAnalsr.templates.noData !== null )
    {
        ...
        $avaAnlsrCodGridList.parent ().append ( html );
    }
    $ ( 'div.ui-jqgrid-bdiv' ).perfectScrollbar ();
}

function codJqGrid ( noDataId )
{
    var $avaAnlsrCodGridList = $ ( '#avaAnlsr_cod_gridList' );

    $avaAnlsrCodGridList.jqGrid ( {
        ...
        loadComplete : function ( data )
        {
            ...
            $ ( '#avaAnlsr_cod_gridList_RowCount' ).text ( resultText );
            ...
            } else
            {
                ...
                var $gridList = $ ( '#avaAnlsr_cod_gridList' );
...
{
    $ ( '#avaAnlsr_cod_gridList' ).trigger ( 'reloadGrid' );
}

// init highcharts
function initCodHighcharts ()
{
    var $avaAnlsrCodGraph = $ ( '#avaAnlsr_cod_graph' );
    var $btnAllAvaAnlsrCod = $ ( '#btn_all_avaAnlsr_cod' );
    var $btnExcelAvaAnlsrCod = $ ( '#btn_excel_avaAnlsr_cod' );

    $.ajax ( {
        url : contextPath + '/hom/analysis/ava/selectAvatyCodChartList.ajax',
        type : 'POST',
        dataType : 'json',
        data : {
            cod : $ ( '#cod' ).val ()
        ...
        success : function ( json )
        {
            ...
            {
                // 기존 차트 삭제
                homUtil.clearHighcharts ( [ $avaAnlsrCodGraph.highcharts () ] );

                if ( json.data != null && json.data.length > 0 )
                {
                    $btnAllAvaAnlsrCod.show ();
                    $btnExcelAvaAnlsrCod.show ();
                ...
                {
                    $btnAllAvaAnlsrCod.hide ();
                    $btnExcelAvaAnlsrCod.hide ();
                    $avaAnlsrCodGraph.html ( '<div class="bg_nodata"><i class="icon_nodata"></i>'
