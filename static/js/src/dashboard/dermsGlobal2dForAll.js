...
function getDashboardInfo()
{
	...
	$.ajax ( {
        url : contextPath + '/hom/dashboard/dermsGlbal/all/getDashboardInfo.ajax',
        type : 'POST',
        dataType : 'json',
...
{
	$.ajax ( {
        url : contextPath + '/hom/dashboard/dermsGlbal/all/getNationDtlInfo.ajax',
        type : 'POST',
        dataType : 'json',
        success : function ( json )
        {
...
{
	$.ajax ( {
        url : contextPath + '/hom/dashboard/dermsGlbal/all/getCntintMarkerDtlInfo.ajax',
        type : 'POST',
        dataType : 'json',
        success : function ( json )
        {
...
function getPvMarkerDtlInfo ( nationId )
{
	...
	$.ajax ( {
        url : contextPath + '/hom/dashboard/dermsGlbal/all/getNationSttusInfo.ajax',
        type : 'POST',
        data : {
        	nationId : nationId
        },  
...
function createPvMarker ( pvMarkerInfo )
{
	...
	if ( pvMarkerInfo.serviceSttus === 'SS04' )
	{
	...
	{
		pvType = pvMarkerInfo.serviceSttus.toLowerCase ();
		pvSttus = 'gry';
		if ( pvMarkerInfo.bizTy == 'BSN01' ) 
		{			
			bizTy = '1';
...
function createNationMarker ( nationMarkerInfo )
{
	var sttus = null;
	var color = null;
	if ( nationMarkerInfo.serviceSttus === 'SS04' )
	{
...
function createCntintMarker ( cntintMarkerInfo )
{
	var sttus = null;
	var color = null;
	if ( cntintMarkerInfo.serviceSttus === 'SS04' )
	{
        ...
        success : function ( json )
        {
            if ( json.status === staticVariable.jsonStatusSuccess )
            {
                if ( dermsDashboardInfo.tpl.pvDtlInfoArea != null )
                {
                    ...
                    if( pvDtlInfo.bizTy == 'BSN01' ) {          //PV
                      contentType = 'type2_pv';
                    } else if( pvDtlInfo.bizTy == 'BSN02' ) {   //ESS
                      contentType = 'type2_ess';                       
                    } else if( pvDtlInfo.bizTy == 'BSN03' ) {   //PV + ESS
                      contentType = 'type4';                    
                    } else if( pvDtlInfo.bizTy == 'BSN04' ) {   //(PV) + ESS   
                      contentType = 'type3' ;                       
                    }
...
{
	$.ajax ( {
        url : contextPath + '/hom/dashboard/dermsGlbal/all/getAcntInfo.ajax',
        type : 'POST',
        dataType : 'json',
        success : function ( json )
        {
