...
function initDatetimepicker ()
{
    ...
    console.log($yyyymmddToDate.val () );
...
// init highcharts
function searchEssAvatyRankChart ()
{
	...
	$("#avatyTopConditionTitle").html("<i class='icon_blt05'></i>"+i18nMessage.msg_avaty+" "+top+ " 1 ~ 10");
	
	var $graph1 = $ ( '#graph1' );
	$.ajax ( {
        url : contextPath + '/hom/advancedanalysis/ess/avaty/selectAdvanAlsEssAvatyRankChartList.ajax',
        type : 'POST',
        dataType : 'json',
        data : $ ( 'form' ).serialize (),
            ...
            if ( json.status === staticVariable.jsonStatusSuccess )
            {
                ...
                if ( json.data.length > 0 )
                {
                    ...
                    $.each ( json.data, function ( index, item )
                    {
                        ...
                        if ( nationId )
                        {
                            // searchRdtnColumnNationChart ( nationId, json.data.length );
                            searchRdtnColumnNationChart ( nationNm, json.data.length );
                        }

                        var avatySeris = {
                            name : item.pvNm,
                            y : homUtil.mathRoundChart ( item.acmsltAvaty, 1 )
                        };
                        seriesArray.push ( avatySeris );
...
//init highcharts
function setFaultCauseTimeChart (pvNm, fault, mntnc)
{
	$("#faultPvNm").empty();
	...
	if(pvNm != 0){
		$("#faultPvNm").html("<i class='icon_blt05'></i>"+pvNm+" " +i18nMessage.msg_causeStopAvaty);
		
		if($.isNumeric ( fault ) && $.isNumeric ( mntnc ) && ( parseFloat(fault) != 0 || parseFloat(mntnc) != 0) ){
			fault = homUtil.mathRoundChart ( fault, 1 );
			mntnc = homUtil.mathRoundChart ( mntnc, 1 );
			$graph2.highcharts ( {
	        	chart: {
	            ...
	            series: [{
	                colorByPoint: true,
	                name : i18nMessage.msg_time,
	                data: [{
	                    name: i18nMessage.msg_maintenanceAvaty,
	                    y: parseFloat(mntnc)
	                }, {
	                    name: i18nMessage.msg_errorAvaty,
	                    y: parseFloat(fault)
	                }]
	            }]
	        });
		}else{
			$("#faultPvNm").html("<i class='icon_blt05'></i> " +i18nMessage.msg_causeStopAvaty);	
			 var noDataMsg = '<div class="bg_nodata"><i class="icon_nodata"></i>'
	             + i18nMessage.msg_sentenceGridNoData + '</div>';
...
function addGroupHeader() {
    ...
    $("#gridList").jqGrid('setGroupHeaders', {
        ...
        groupHeaders:[
			{startColumnName: 'goalAvaty', numberOfColumns: 2, titleText: i18nMessage.msg_avaty+'(%)'},
        ]
    });
}
...
function jqGridBasic() {
    ...
    $gridList.jqGrid ( {
        url : contextPath + '/hom/advancedanalysis/ess/avaty/selectAdvanAlsEssAvatyRankGridList.ajax',
        mtype : 'POST',
        datatype : 'json',
        height : 312,
        autowidth : true,
        ...
        colNames : [ i18nMessage.msg_rank, i18nMessage.msg_siteName, i18nMessage.msg_area,
                ...
                i18nMessage.msg_totalStopTime + '(h)', i18nMessage.msg_maintenanceTime + '(h)', 
                i18nMessage.msg_errorOccurTime + '(h)'],
        colModel : [
                {
                    name : 'rank',
                    align : 'center',
        ...
        loadComplete : function ( data )
        {
            ...
            if ( data.records === 0 )
            {
                ...
                setFaultCauseTimeChart (data.records);
            } else
            {
                $gqNodata.hide ();
                
                setFaultCauseTimeChart (data.rows[0].pvNm, data.rows[0].faultTm, data.rows[0].mntncTm ); 
        ...
        onSelectRow : function ( rowId, status )
        {
            ...
            setFaultCauseTimeChart (rowData.pvNm, rowData.faultTm, rowData.mntncTm );           
        },
            ...
            if ( chartImgInfoMap.size () == 1 )
            {
                ...
                if ( $excelUrl1 )
                {
                    $ ( 'form' ).prepend ( $excelUrl1 ).prop ( 'action',
                            contextPath + '/hom/excel/advancedanalysis/ess/avaty/download.do' ).submit ();
                    $excelUrl1.remove ();
                } else
                {
                    $.customizeDialog ( {
