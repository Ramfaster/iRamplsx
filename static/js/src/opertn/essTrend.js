...
function customizeForm ()
{
    ...
    var $selectType4 = $ ( '.select4' ).customizeSelect ( {
        width : 210,
        paddingHorizontal : 15,
        height : 30,
        color : '#3c3c3c',
...
function initCustomizeScroll($box1Equipment)
{
	$box1Equipment.unbind('select2:open');
		 
	var selectFlag1 = true;
	
    // select event
	$box1Equipment.on ( 'select2:open', function ( e )
    {
        if ( selectFlag1 )
        {
...
function initEquipmentSelect ()
{
    ...
    var eqmtTpl = getTemplate ( templates.eqmtInfoOption );
    
    
    var $box1Equipment = $ ( '#box1_equipment' );
    var $box1Parameter = $ ( '#box1_parameter' );
    $ ( '#box1_classification' ).change ( function ( event )
    {
        var params = {
            bizTy : $ ( ':selected', $ ( this ) ).val ()
        };
        getEqmtInfoList ( params, $box1Equipment, eqmtTpl );

        if ( $box1Equipment.find ( 'option' ).length > 1 )
        {
        	$box1Equipment.find ( 'option' ).eq ( 0 ).prop ( "selected", "selected" ).trigger ( 'change' );
        } else
        {
        	$box1Equipment.val ( '' ).trigger ( 'change' );
        }
    ...
    $ ( '#box1_classification' ).trigger ( 'change' );
    ...
    var $box2Equipment = $ ( '#box2_equipment' );
    var $box2Parameter = $ ( '#box2_parameter' );
    $ ( '#box2_classification' ).change ( function ( event )
    {
        var params = {
            bizTy : $ ( ':selected', $ ( this ) ).val ()
        };
        getEqmtInfoList ( params, $box2Equipment, eqmtTpl );

        if ( $box2Equipment.find ( 'option' ).length > 1 )
        {
        	$box2Equipment.find ( 'option' ).eq ( 0 ).prop ( "selected", "selected" ).trigger ( 'change' );
        } else
        {
        	$box2Equipment.val ( '' ).trigger ( 'change' );
        }
    ...
    $ ( '#box2_classification' ).trigger ( 'change' );
...
// 설비 정보 가져오기
function getEqmtInfoList ( params, $selectEquipment, tpl )
{
    $.ajax ( {
        url : contextPath + '/hom/operation/trend/selectEqmtList.ajax',
        type : 'POST',
        data : params,
        dataType : 'json',
        async : false,
            ...
            if ( json.status === staticVariable.jsonStatusSuccess )
            {
                if ( tpl !== null )
                {
                    ...
                    var html = template ( {
                        ...
                        eqmtInfoVOList : json.data
                    } );

                    $selectEquipment.empty ().html ( html );                    
                    initCustomizeScroll($selectEquipment);
        ...
        success : function ( json )
        {
            if ( json.status === staticVariable.jsonStatusSuccess )
            {
                if ( tpl !== null )
                {
                    ...
                    initCustomizeScroll($selectParamtr);
                    // $selectParamtr.prop ( "selectedIndex", 1 ).trigger ( 'change' );
                }
            } else if ( json.status === staticVariable.jsonStatusError )
            {
...
{
    var tpl = getTemplate ( templates.essItemInfoLi );

    $ ( '#btn_add_item01' ).on ( 'click', function ()
    {
    	var $box1Classification = $ ( '#box1_classification option:selected' );
        var $box1Equipment = $ ( '#box1_equipment option:selected' );
        var $box1Parameter = $ ( '#box1_parameter option:selected' );
        
    	// 설비 선택 여부 체크
    	if($box1Equipment.val() == ""){
    		 $.customizeDialog ( {
                 template : templates.dialog,
                 message : i18nMessage.msg_alertEqmtNoSelect,
                 checkText : i18nMessage.msg_ok,
                 cancelText : i18nMessage.msg_cancel,
                 type : staticVariable.dialogTypeInfo
             } );
    	...
    	// 항목 선택 여부 체크
    	if($box1Parameter.val() == ""){
   		 $.customizeDialog ( {
                template : templates.dialog,
                message : i18nMessage.msg_alertParameterNoSelect,
                checkText : i18nMessage.msg_ok,
                cancelText : i18nMessage.msg_cancel,
                type : staticVariable.dialogTypeInfo
            } );
...
function searchData ( tagIdArray, graphNum )
{
    ...
    $.ajax ( {
        url : contextPath + '/hom/operation/trend/selectGraphDataForEss.ajax',
        type : 'POST',
        data : params,
        dataType : 'json',
        success : function ( json )
                ...
                _.each ( json.data, function ( seiresData, i )
                {
                    ...
                    _.each ( seiresData.eqmtTrendValues, function ( eqmtTrendValue, i )
                    {
                        data.push ( [ homUtil.convertDateStringToLong ( eqmtTrendValue.occrrncDt ),
                                homUtil.mathFloor ( eqmtTrendValue.tagVal, staticVariable.decimalPoint ) ] );
                        chkData++;
                    } );
