...
function customizePopJqgrid ()
{
    ...
    var strSort = null;
    var chkInit = true;
    colNames = [ "acntId", i18nMessage.msg_grade, i18nMessage.msg_corporateCharger, i18nMessage.msg_email,
            i18nMessage.msg_phoneNumber ];
    ...
    $ ( '#gridListPop' )
            .jqGrid (
                    {
                        url : contextPath + '/hom/sysmgt/scheduler/selectSchedulerrReceiveSystemUserList.ajax',
                        mtype : 'POST',
                        datatype : 'json',
                        height : 360,
                        autowidth : true,
                        ...
                        loadComplete : function ( data )
                        {
                            ...
                            } else
                            {
                                ...
                                var $gridList = $ ( '#gridListPop' );
                                setSelectedUser ( $gridList, chkInit );
                                if ( chkInit )
                                {
                                    chkInit = false;
                                }
...
function selectCharger ()
{
    ...
    $btnReg.click ( function ()
    {
        ...
        if ( chkCount > 0 )
        {
            ...
            {
                userCnt++;
                arrChargerId.push ( $ ( this ).attr ( "data-id" ) );
                if ( index == 0 )
                {
                    firstUserNm = $ ( this ).attr ( "data-nm" );
                }
            } );
            if ( type == "M" )
            {
                ...
                $ ( "#mailInboundSystemUserPopup" ).attr (
                        "href",
                        contextPath + "/hom/sysmgt/scheduler/batchSchdulMgtFormPopupSysUser.do?type=M&operIdArray="
...
function searchPopJqgrid ()
{
    ...
    var $icon_search_btn = $ ( '#icon_search_btn' );
    ...
    {
        reloadPopJqgrid ( $gridList, $searchValue );
    } );
    ...
    $icon_search_btn.click ( function ( event )
    {
        reloadPopJqgrid ( $gridList, $searchValue );
    } );
}
// jqgrid reload
function reloadPopJqgrid ( $gridList, $searchValue )
{
...
// 기존 선택된 사용자 선택하기
function setSelectedUser ( $gridList, chkInit )
{
    if ( chkInit )
    {
