                    ...
                    $.each ( json.data,
                            function ( index, item )
                            {
                                ...
                                powerArray.push ( [ targetDate,
                                        homUtil.mathRoundChart ( item.generation, 1 ) ] );
...
function getPvInfo ()
{
    ...
    var $codAvgPwPrdtTime = $ ( '#pvAvgGeneTime1' );
        ...
        success : function ( json )
        {
            if ( json.status === staticVariable.jsonStatusSuccess )
            {
                ...
                if ( pvInfo != null )
                {
                    ...
                    // 평균발전시간
                    $avgPwPrdctTime.text ( homUtil.mathRoundComma(pvStdrInfoVO.avgPwPrdtTime,2) );
                    $avgPwPrdctTime.parent ().find ( '.unit01' ).text ( '(' + pvStdrInfoVO.avgPwPrdtTimeUnit + ')' );
                    // 누적발전시간
                    $codAvgPwPrdtTime.text ( homUtil.mathRoundComma(pvStdrInfoVO.codAvgPwPrdtTime,2) );
                    $codAvgPwPrdtTime.parent ().find ( '.unit01' ).text ( '(' + pvStdrInfoVO.avgPwPrdtTimeUnit + ')' );
...
function createUpAndDownArrow ( stdrValue, ptValue )
{
    ...
    html = homUtil.mathRoundComma ( stdrValue, staticVariable.decimalPoint ) + html;
...
function getTotalKpiInfo ()
{
    ...
    $.ajax ( {
        url : contextPath + '/hom/monitoring/ess/pvess/selectPvEssKpi.ajax',
        type : 'POST',
        dataType : 'json',
        success : function ( json )
        {
            if ( json.status === staticVariable.jsonStatusSuccess )
            {
            	 if ( json.data !== null )
                 {
	                 ...
	                 var gene = homUtil.mathRoundComma (json.data.goalGeneqty,1) 
	 		 			+"<span class='per' style='color:"+ json.data.geneqtyHexCd+"'>"
	 		 			+"(<span class='arw'>"+json.data.geneqtyUnicode+"</span>"
	 		           +homUtil.mathRoundComma (json.data.geneqtyRate, 1)+"%)</span>";
	                 
	                 if(json.data.geneqtyUnitNm != null)
	                 {
	                	 $("#kpiEnergyUnit").text("("+json.data.geneqtyUnitNm+"h)");
	                 }else
	                 {
	                	 $("#kpiEnergyUnit").text("(kWh)");
	                 }
	                 
	                 
	                 $ ( '#kpi_gene' ).text(homUtil.mathRoundComma (json.data.acmsltGeneqty, 1));
	                 $ ( '#kpi_gene_cng').html(gene);
	                 
	                 var rdtn = homUtil.mathRoundComma (json.data.goalRdtn,1) 
	 		 			+"<span class='per' style='color:"+ json.data.rdtnHexCd+"'>"
	 		 			+"(<span class='arw'>"+json.data.rdtnUnicode+"</span>"
	 		           +homUtil.mathRoundComma (json.data.rdtnRate, 1)+"%)</span>";
	                  
	                 $ ( '#kpi_rdt' ).text(homUtil.mathRoundComma (json.data.acmsltRdtn, 1));
	                 $ ( '#kpi_rdt_cng').html(rdtn);
	                 
	                 if(json.data.rdtnUnitNm != null)
	                 {
	                	 $("#kpiRdtnUnit").text("("+json.data.rdtnUnitNm+"h/㎡/d)");
	                 }else
	                 {
	                	 $("#kpiEnergyUnit").text("(Wh/㎡/d)");
	                 }
	                 
	                 
	                 var pr = homUtil.mathRoundComma (json.data.goalPr,1) 
	 		 			+"<span class='per' style='color:"+ json.data.prHexCd+"'>"
	 		 			+"(<span class='arw'>"+json.data.prUnicode+"</span>"
	 		           +homUtil.mathRoundComma (json.data.prRate, 1)+"%)</span>";
	                 
	                 $ ( '#kpi_Pr' ).text(homUtil.mathRoundComma (json.data.acmsltPr, 1));
	                 $ ( '#kpi_Pr_cng').html(pr);
    ...
    $.ajax ( {
        ...
        success : function ( json )
        {
            if ( json.status === staticVariable.jsonStatusSuccess )
            {
                ...
                //TODO : #pvAlarmSttus 설비상태별로 가져와야 함.
                
                var pvAlarmText;
                var pvAlarmLevel = json.data.pvAlarmGradCd;
                var pvAlarm;
                
                switch ( pvAlarmLevel )
                {
                    case 'ALV01':
                        pvAlarmText = i18nMessage.msg_mntrNormal;
                        pvAlarm = "normal";
                        break;
                    case 'ALV02':
                        pvAlarmText = i18nMessage.msg_mntrEqmtWarning;
                        pvAlarm = "warnng";
                        break;
                    case 'ALV03':
                        pvAlarmText = i18nMessage.msg_mntrFault;
                        classNm = "broken";
                        break;
                    case 'ALV04':
                        pvAlarmText = i18nMessage.msg_mntrNetErr;
                        pvAlarm = "neterr";
                        break;
                    default:
                        pvAlarmText = i18nMessage.msg_mntrNormal;
                ...
                $("#pvAlarmSttus").text(pvAlarmText);
                $("#pvAlarmSttus").attr('class', 'dvc_status '+pvAlarm);
                ...
                $ ( '#pgpVal' ).text(homUtil.mathRoundComma (pgpVal, 1));   
                
                if($("#pvInfo1").hasClass("off")){
                	$ ( '#pgpVal' ).html(neterrIcon+homUtil.mathRoundComma (pgpVal, 1));
                }else{
                	$ ( '#pgpVal' ).text(homUtil.mathRoundComma (pgpVal, 1));
                }
                
                var pgpValNum = Number ( pgpVal );
                if(pgpValNum > 0){
                	$ ( '#pvOperationSttus' ).text(i18nMessage.msg_PowerRun);
                	$ ( '#pvOperationSttus' ).attr('class', "dvc_status elegan" );
                }else{
                	$ ( '#pvOperationSttus' ).text(i18nMessage.msg_Waiting);
                	$ ( '#pvOperationSttus' ).attr('class', "dvc_status waitng" );
                }
...
//알람 상태 
function getPvEssAlarmStatus (  )
{
	$.ajax ( {
        url : contextPath + '/hom/monitoring/ess/pvess/selectPvEssAlarmStatus.ajax',
        type : 'POST',
        dataType : 'json',
        async : false,
        success : function ( json )
        {
            if ( json.status === staticVariable.jsonStatusSuccess )
            {
                ...
                var pvAlarm = json.data.PV;
                var bizTy = json.data.BIZ_TY;
                
                //BSN04 site 는  PV 알람은 가져 올 필요 없음.장비 단위(PV계전기)만 유효
                if(bizTy == "BSN04") {
                    $("#pvAlarm").attr('class', 'dvc_status neterr');
                } else {
                    $("#pvAlarm").attr('class', 'dvc_status '+pvAlarm);                    
                }
