...
function searchAvaty ()
{
    ...
    $ ( '#btnSubmit' ).on ( 'click', function ( event, initFlag )
    {
        ...
        searchCodMonitoringInfo (toDate);
        getAvatyChartList ();
...
// 모니터링 > ESS 가동률 상단 COD - 종료일 정보 조회
function searchCodMonitoringInfo (toDate)
{
    var params = {
    ...
    $
            .ajax ( {
                url : contextPath + '/hom/monitoring/ess/ava/selectCodMonitoringInfo.ajax',
                type : 'POST',
                dataType : 'json',
                data : params,
                success : function ( json )
                {
                    ...
                    {
                        $ ( '#goalAvaty' ).text (
                                homUtil.mathRoundComma ( json.data.goalAvaty, staticVariable.decimalPoint ) );
                        $ ( '#acmsltAvaty' ).text (
                                homUtil.mathRoundComma ( json.data.acmsltAvaty, staticVariable.decimalPoint ) );
...
function getAvatyChartList ()
{
    ...
    $.ajax ( {
        url : contextPath + '/hom/monitoring/ess/ava/selectAvatyChartList.ajax',
        type : 'POST',
        dataType : 'json',
        data : $ ( "form" ).serialize (),
        success : function ( json )
        {
            if ( json.status === staticVariable.jsonStatusSuccess )
            {
                ...
                if ( json.data.length > 0 )
                {
                    ...
                    var yieldTmUnitNm = json.data[0].yieldTmUnitNm;
                    ...
                    $.each ( json.data, function ( index, item )
                    {
                        ...
                        yieldTmArray.push ( [ targetDate,
                                homUtil.mathRoundChart ( item.yieldTm, 1 ) ] );
                    } );
                    ...
                    var avaMaxArray = [];
                    avaMaxArray.push ( _.max ( _.pluck ( goalAvatyArray, [ 1 ] ) ) );
                    avaMaxArray.push ( _.max ( _.pluck ( beforeYearAvatyArray, [ 1 ] ) ) );
                    avaMaxArray.push ( _.max ( _.pluck ( acmsltAvatyArray, [ 1 ] ) ) );
                    
                    var avatmMaxArray = [];
                    avatmMaxArray.push ( _.max ( _.pluck ( yieldTmArray, [ 1 ] ) ) );
                    ...
                    var yieldTmYaxis = {
                        max :  _.max ( avatmMaxArray ),
                        opposite : true,
                        title : {
                            text : i18nMessage.msg_driveTime + homUtil.wrapWord ( yieldTmUnitNm, '(', ')' )
                        }
                    };
                    ...
                    $ ( '#graph1' ).highcharts ( {
                        ...
                        series : [ {
                            ...
                            type : 'line',
                            yAxis : 1,
                            name : i18nMessage.msg_driveTime,
                            data : yieldTmArray
                        } ]
                    } );
                } else
...
function searchAvatyJqgrid ( $gridList, tpl )
{
    ...
    $ ( '#gridList' ).jqGrid (
            {
                url : contextPath + '/hom/monitoring/ess/ava/selectAvatyGridList.ajax',
                mtype : 'POST',
                datatype : 'json',
                height : 175,
                autowidth : true,
                ...
                colNames : [ dateTypeText, i18nMessage.msg_goal, i18nMessage.msg_beforeYear, i18nMessage.msg_acmslt,
                        i18nMessage.msg_goal, i18nMessage.msg_beforeYear, i18nMessage.msg_driveTime, 'avaUnitNm',
                        'yieldTmUnitNm' ],
                colModel : [
                        {
                            name : 'stdrDate',
                        ...
                        },
                        {
                            ...
                            formatter : function ( cellvalue, options, rowObject )
                            {
                                return numberRoundComma ( cellvalue, 2 );
                            }
                        }, {
                            name : 'avaUnitNm',
                            hidden : true
                ...
                $.ajax ( {
                    ...
                    success : function ( data )
                    {
                        ...
                        $ ( 'form' ).prepend ( $excelUrl, $menuName ).prop ( 'action',
                                contextPath + '/hom/excel/monitoring/avaty/ess/download.do' ).submit ();
