...
function selectChargyMonitoringInfo (toDate)
{
    ...
    $.ajax ( {
        url : contextPath + '/hom/monitoring/ess/efcny/selectChargyMonitoringInfo.ajax',
        type : 'POST',
        dataType : 'json',
        data : params,
        success : function ( json )
        {
            ...
            {
                $ ( '#goalChar' ).text ( homUtil.mathRoundComma ( json.data.goalChar, staticVariable.decimalPoint ) );
                $ ( '#acmsltChar' )
                        .text ( homUtil.mathRoundComma ( json.data.acmsltChar, staticVariable.decimalPoint ) );
...
function getAvatyChartList ()
{
    ...
    $.ajax ( {
        url : contextPath + '/hom/monitoring/ess/efcny/selectChargyChartList.ajax',
        type : 'POST',
        dataType : 'json',
        data : $ ( "form" ).serialize (),
        success : function ( json )
                ...
                if ( json.data.length > 0 )
                {
                    ...
                    $.each ( json.data,
                            function ( index, item )
                            {
                                ...
                                effectArray.push ( [ targetDate,
                                        homUtil.mathRoundChart ( item.effect, 1 ) ] );
                                beforeYeareffectArray.push ( [ targetDate,
                                                     homUtil.mathRoundChart ( item.beforeYearAvaty, 1 ) ] );
                                goalgapArray.push ( [ targetDate,
                                        homUtil.mathRoundChart ( item.goalGap, 1 ) ] );
                            } );
                    ...
                    avaMinArray.push ( _.min ( _.pluck ( effectArray, [ 1 ] ) ) );

                    var avaMaxArray = [];
                    avaMaxArray.push ( _.max ( _.pluck ( chargyArray, [ 1 ] ) ) );
                    avaMaxArray.push ( _.max ( _.pluck ( dischargyArray, [ 1 ] ) ) );
                    ...
                    $ ( '#graph1' ).highcharts ( {
                        ...
                        series : [ {
                            ...
                            dashStyle : 'ShortDash',
                            yAxis : 1, // 하이차트 우측 설정
                            name : i18nMessage.msg_goal + ' ' + i18nMessage.msg_effctiveness,
                            data : goalArray
                        }, {
                            type : 'line',
                            yAxis : 1,// 하이차트 우측 설정
                            name : i18nMessage.msg_beforeYear + ' ' + i18nMessage.msg_effctiveness,
                            data : beforeYeareffectArray
                        }, {
                            type : 'line',
                            yAxis : 1,// 하이차트 우측 설정
                            name : i18nMessage.msg_acmslt + ' ' + i18nMessage.msg_effctiveness,
                            data : effectArray
                        }  ]
                    } );
                } else
    ...
    $ ( '#gridList' ).jqGrid (
            {
                ...
                colNames : [ dateTypeText, i18nMessage.msg_chargy, i18nMessage.msg_dischargy, i18nMessage.msg_goal,
                             i18nMessage.msg_beforeYear, i18nMessage.msg_acmslt, i18nMessage.msg_goal, i18nMessage.msg_beforeYear ],
                colModel : [
                        {
                            name : 'stdrDate',
                            align : 'center',
                ...
                $.ajax ( {
                    ...
                    success : function ( data )
                    {
                        ...
                        $ ( 'form' ).prepend ( $excelUrl, $eqmtNm, $menuName ).prop ( 'action',
                                contextPath + '/hom/excel/monitoring/ava/char/download.do' ).submit ();
