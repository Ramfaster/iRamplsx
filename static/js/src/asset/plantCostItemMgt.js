...
function customizeJqgrid ()
{
    ...
    $ ( '#gridList' ).jqGrid (
            {
                url : contextPath + '/hom/asset/plantCostItem/list.ajax',
                mtype : 'POST',
                datatype : 'json',
                height : 642,
                autowidth : true,
                ...
                onSelectRow : function ( rowId, status )
                {
                    ...
                    location.href = contextPath + '/hom/asset/plantCostItem/view.do?itemId=' + rowData.pvId + ''
                            + rowData.itemId + '&parntsItemId=' + rowData.parntsItemId + '&pvId=' + rowData.pvId
...
function reloadJqgrid ( treeNode )
{
    ...
    var postData = null;
    // alert ( treeNode.pId + ' ' + treeNode.pvId );
    paramPvId = treeNode.mdlPid;

    postData = {
        itemId : treeNode.id,
        pvId : treeNode.mdlPid
    };
    ...
    // paramItemId = 'nochild';
    // }
    // alert ( postData.itemId + ' ' + postData.pvId );
            ...
            $.when ( $.customizeDialog ( {
                ...
                type : staticVariable.dialogTypeConfirm
                    ...
                    function ( confirm )
                    {
                        if ( confirm )
                        {
                            location.href = $that.attr ( 'href' ) + '?itemId=' + itemIdArray.toString () + '&pvId='
                                    + paramPvId + '&parntsItemId=' + paramParntsItemId;
                        }
                    } );
            // } else
            // {
    ...
    $ ( '#btn_update_order' ).on ( 'click', function ()
    {
        ...
        } else
        {
            ...
            } ) ).then ( function ( confirm )
            {
                ...
                {
                    $.ajax ( {
                        url : contextPath + '/hom/asset/plantCostItem/updateItemOrder.ajax',
                        type : 'POST',
                        dataType : 'json',
                        data : {
...
function checkBtnAdd ()
{
    ...
    $btnAdd.on ( 'click', function ()
    {
        ...
        // {
        location.href = $ ( this ).attr ( 'href' ) + '&pvId=' + paramPvId + '&itemId=' + paramItemId + '&parntsItemId='
                + paramParntsItemId + '&searchKey=' + $ ( ':selected', $ ( "#searchKey" ) ).val () + '&searchKeyword='
