function btncopyitemcheck ()
{
    if ( typeof $ ( "#btn_copy_popup_exec" ) !== 'undefined' )
    {
        ...
        {
            var params = {
                popupCopySpcId1 : $ ( ':selected', $ ( '#popupCopySpcId1' ) ).val (),
                popupCopySpcId2 : $ ( ':selected', $ ( '#popupCopySpcId2' ) ).val ()
            };

            $.ajax ( {
                url : contextPath + '/hom/asset/companyCostItem/copyItemPopup.ajax',
                type : 'GET',
                data : params,
                dataType : 'json',
...
{
    var $dateType1 = $ ( '#popupCopySpcId1' ).customizeSelect ( {
        width : 150,
    ...
    var $dateType1 = $ ( '#popupCopySpcId2' ).customizeSelect ( {
        width : 150,
        paddingHorizontal : 15,
        height : 30,
        color : '#3c3c3c',
    ...
    $ ( "#popupCopySpcId1" ).on ( 'change', fnPopupCopyTextValue );
    $ ( "#popupCopySpcId2" ).on ( 'change', fnPopupCopyTextValue );
}

function fnPopupCopyTextValue ()
{
    $ ( "#div_pop_subtxt01" ).text (
            $ ( ':selected', $ ( '#popupCopySpcId1' ) ).text () + ' > '
                    + $ ( ':selected', $ ( '#popupCopySpcId2' ) ).text () + ' ' + i18nMessage.msg_sentenceAssetCopy );
}
