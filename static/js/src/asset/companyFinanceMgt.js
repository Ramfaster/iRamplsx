var selectCompanyFinanceMgtList = [];
var chkcell = {
    cellId : undefined,
    chkval : undefined
}; // cell rowspan 중복 체크
...
function customizeJqgrid ()
{
    ...
    $ ( '#gridList' ).jqGrid (
            {
                url : contextPath + '/hom/asset/companyFinance/list.ajax',
                mtype : 'POST',
                datatype : 'json',
                height : 575,
                autowidth : true,
                ...
                onSelectRow : function ( rowId, status )
                {
                    ...
                    if ( status )
                    {
                        ...
                        selectCompanyFinanceMgtList.push ( rowDataObj );
                    } else
                    {
                        var idx = selectCompanyFinanceMgtList.indexOf ( rowId );
                        selectCompanyFinanceMgtList.splice ( idx, 1 );
                    }
                },
                ...
                onSelectAll : function ( aRowids, status )
                {
                    ...
                    if ( status )
                    {
                        ...
                        {
                            var chkInclude = selectCompanyFinanceMgtList.indexOf ( aRowids[i] );
                            if ( chkInclude < 0 )
                            {
                                ...
                                selectCompanyFinanceMgtList.push ( rowDataObj.tagId );
                            }
                        }
                    ...
                    } else
                    {
                        for ( var i = 0; i < aRowids.length; i++ )
                        {
                            ...
                            if ( chkInclude > -1 )
                            {
                                ...
                                var idx = selectCompanyFinanceMgtList.indexOf ( rowDataObj.tagId );
                                selectCompanyFinanceMgtList.splice ( idx, 1 );
                            }
                        }
                    }
...
function searchJqgrid ()
{
    ...
    $btnSearch.on ( 'click', function ( event, initFlag )
    {
...
function reloadJqgrid ( $gridList )
{
    ...
    selectCompanyFinanceMgtList.length = 0;
    ...
    $ ( '#btn_save' ).click ( function ()
    {
        ...
        if ( chkValidate == 0 )
        {
            ...
            } ) ).then ( function ( confirm )
            {
                ...
                {
                    $.ajax ( {
                        url : contextPath + '/hom/asset/companyFinance/saveSpcPlanAcmslt.ajax',
                        type : 'POST',
                        dataType : 'json',
                        async : false,
                        data : {
...
function getNationSpcInfo ()
{
    ...
    $nation.on ( 'change', function ( event )
    {
        ...
        $.ajax ( {
            url : contextPath + '/hom/asset/companyFinance/selectMainSpcInfoList.ajax',
            type : 'POST',
            data : params,
            dataType : 'json',
            async : false,
